class SeriesController < ApplicationController
  ...
  def index
    if params["actor"]
      actor = Actor.where("name like ?", "#{params["actor"]} %").first
      @series = actor.present? ? actor.series.includes(:reviews) : []
    else
      @series = Series.includes(:reviews).all
    end
  end
  ...
  private
    ...
    def series_params
      params.require(:series).permit(:name, :genre, :no_of_seasons, :director, :date_of_first_release)
    end
end
